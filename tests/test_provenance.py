...
def test_provenance_generation(test_environment):
    ...
    assert (item1_snapshot, RDF.type, PROV.Entity) in item1_prov_graph, (
        "item1 snapshot is not typed as prov:Entity"
    )
    assert (item2_snapshot, RDF.type, PROV.Entity) in item2_prov_graph, (
        "item2 snapshot is not typed as prov:Entity"
    )
    ...
    ) in item1_prov_graph, "item1 snapshot missing primary source"
    assert (
        item2_snapshot,
        PROV.hadPrimarySource,
        URIRef(primary_source),
    ) in item2_prov_graph, "item2 snapshot missing primary source"
...
ex:item3 a crm:E22_Human-Made_Object ;
    ...
    assert (item3_snapshot, RDF.type, PROV.Entity) in item3_prov_graph, (
        "item3 snapshot is not typed as prov:Entity"
    )
