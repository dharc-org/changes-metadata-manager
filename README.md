...
The `folder_metadata_builder.py` script processes the SharePoint folder structure and generates metadata and provenance files for each stage.

Prerequisites:
- SharePoint structure JSON file (generated by `sharepoint_extractor.py`)
- Knowledge graph in Turtle format (`data/kg.ttl`)

Usage:

```bash
python -m aldrovandi_provenance.folder_metadata_builder [-o OUTPUT_DIR]
```

Arguments:
- `-o`, `--output`: Output directory for generated files (default: `data/output`)

The script creates a directory structure mirroring the SharePoint folders, with each stage containing:
